// Package rnbit implements the bridge interfaces for rnbit blockchain.
package rnbit
...
// Bridge rnbit bridge inherit from eth bridge
type Bridge struct {
	*eth.Bridge
}

// NewCrossChainBridge new rnbit bridge
...
func (b *Bridge) VerifyChainID() {
	...
	targetChainID := eth.GetChainIDOfNetwork(eth.RnbitNetworkAndChainIDMap, networkID)
	isCustom := eth.IsCustomNetwork(networkID)
	if !isCustom && targetChainID == nil {
		log.Fatalf("unsupported Rainbit network: %v", b.ChainConfig.NetID)
	}
